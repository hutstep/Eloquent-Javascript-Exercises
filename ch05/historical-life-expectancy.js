...
const groupBy = (arr, grpName) => {
  let groupObj = {};
  arr.forEach(element => {
    let groupName = grpName(element);
    groupName in groupObj ? groupObj[groupName].push(element) :
                                                groupObj[groupName] = [element];
  });
  return groupObj;
};

const getAvgAgeByCentury = agesObj => {
  for (let century in agesObj) {
    if (!agesObj.hasOwnProperty(century)) continue;
    agesObj[century] = average(agesObj[century].map(person => {
...
let avgAgesByCentury = getAvgAgeByCentury(groupBy(ancestry, getCentury));
for (let century in avgAgesByCentury) {
  if (!avgAgesByCentury.hasOwnProperty(century)) continue;
  console.log(century + ':', avgAgesByCentury[century]);
}
